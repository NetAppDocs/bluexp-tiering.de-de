...
Wenn Sie eine Schätzung erhalten möchten, fügen Sie einfach Ihren ONTAP Cluster zu BlueXP hinzu und prüfen Sie ihn über die Seite zu Cloud Tiering-Clustern. Klicken Sie auf *Berechnen Sie mögliche Tiering-Einsparungen* für den Cluster, um den zu starten https://bluexp.netapp.com/cloud-tiering-service-tco["TCO-Rechner für Cloud Tiering"^] Um zu sehen, wie viel Geld Sie sparen können.
...
Ja. Die Connector-Software kann heruntergeladen und manuell auf einem Linux-Host in Ihrem Netzwerk installiert werden. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-install-connector-on-prem.html["Lesen Sie, wie Sie den Connector in Ihrem Haus installieren"].
